    ...
    public void handle(CAddStandToWorldData message, Supplier<Context> ctx) {
        if (ctx.get().getDirection() == NetworkDirection.PLAY_TO_SERVER) {
            ctx.get().enqueueWork(() -> {
                ...
                if (world != null)
                    if (!world.isRemote) {
                        Objects.requireNonNull(world.getServer()).getWorld(world.getDimensionKey());
                        StandPerWorldCapability.getLazyOptional(world.getServer().getWorld(world.getDimensionKey())).ifPresent(uniqueStandHandler -> {
